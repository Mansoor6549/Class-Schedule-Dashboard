...
function showNotes() {
    ...
    notesObj.forEach(function (element, index) {
        ...
        <div>
        <div class="card">
            <h2>Teacher:${element.textArea} </h2>
            <p>Batch:${element.title}</p>
            <button onClick="deleteNote(this.id)" id="${index}" class="deleteBtn">Delete</button>
        </div>
        </div>`;
    });
    ...
    if (notesObj.length == 0) {
        ...
        insertNotes.innerHTML = ` Please click on "Schedule Batch" button to add a new class.`
